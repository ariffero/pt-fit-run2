...
void doOneDataFit(TTree *dataTree, TFile *saveFile, float binID[3])
{
  ...
  RooRealVar pt("fMuMuPt","pt",minPt,maxPt);
  RooRealVar mass("fMuMuM","m_{#mu#mu} (GeV/c^{2})",minMass,maxMass);
  RooRealVar rapidity("fMuMuY","rapidity",minRapidity,maxRapidity);

  // import the tree
  RooDataSet inData("inData", "inData", RooArgSet(pt, mass, rapidity), Import(*dataTree));
  ...
  // save a PDF with the fit results
...
void fitJPsiInPtBins(vector<double> &ptFitBinning, int ptId, double massRange[], double rapidityRange[], string identifier)
{
  ...
  // do not show canvas pop up
  if(notShow) gROOT->SetBatch(kTRUE);

  // identifier of the parameters = name of the config file
  gIdentifier = identifier;

  // neutron class and phi bins -> not really used here
  ...
  dataFile = new TFile("PbPbFwdRun2/Data/LHC18_AnalysisResults.root");
  dataTree = (TTree *) dataFile->Get("NanoMUONCMUP6/fRecTree");
  
  // chose kinematics for the fit
  ...
  // allow the macro to run on different phi bins: use just one bin for this study
	// loop on phi bins
