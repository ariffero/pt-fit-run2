...
void ptFit(vector<double> &ptFitBinning, bool yyTemplate, bool binned, bool isChi2Fit, double massRange[], double rapidityRange[], string configName)
{
  ...
  if(!gYyTemplate){
    ...
    fData = new TFile("PbPbFwdRun2/Data/LHC18_AnalysisResults.root");
    dataTree = (TTree*)fData->Get("NanoMUONCMUP6/fRecTree");
  }

  //-----Read the files with the MC----------------------
  TFile *fCohJpsi = new TFile("PbPbFwdRun2/MC/CohJpsiToMu_AnalysisResults.root");
  TTree *jpsiCohTree = (TTree*)fCohJpsi->Get("NanoMUONCMUP6/fRecTree");

  TFile *fIncohJpsi = new TFile("PbPbFwdRun2/MC/IncohJpsiToMu_AnalysisResults.root");
  TTree *jpsiIncohTree = (TTree*)fIncohJpsi->Get("NanoMUONCMUP6/fRecTree");

  TFile *fCohpsi2sFd = new TFile("PbPbFwdRun2/MC/CohPsi2sToMuPi_AnalysisResults.root");
  TTree *psi2sFdCohTree = (TTree*)fCohpsi2sFd->Get("NanoMUONCMUP6/fRecTree");

  TFile *fIncohpsi2sFd = new TFile("PbPbFwdRun2/MC/IncohPsi2sToMuPi_AnalysisResults.root");
  TTree *psi2sFdIncohTree = (TTree*)fIncohpsi2sFd->Get("NanoMUONCMUP6/fRecTree");

  TTree *mumuTree = NULL;
  TFile *fMumu = NULL;
  if(gYyTemplate){
    fMumu = new TFile("PbPbFwdRun2/MC/TwoGammaToMuMedium_AnalysisResults.root");
    mumuTree = (TTree*)fMumu->Get("NanoMUONCMUP6/fRecTree");
  }
  
  // Construct the RooDataSet
  // Start defining the RooRealVar
  RooRealVar pt("fMuMuPt","p_{T} (GeV/c)",gPtFitBinning[0],gPtFitBinning[gPtFitBinning.size()-1]);
  RooRealVar mass("fMuMuM","m_{#mu#mu} (GeV/c^{2})", massRange[0], massRange[1]);
  RooRealVar rapidity("fMuMuY","rapidity", rapidityRange[0], rapidityRange[1]);
                  ...
                  RooDataSet mumu, RooRealVar pt)
{
  ...
  */
  ...
  // compute and add the chi2 to the plot
  TLatex latex;
  latex.SetNDC();
  ...
  TFile *saveFile = new TFile(Form("results-pt-fit-%s.root",gConfigName.c_str()),"recreate");
  cPt->Write();
  cCm->Write();
  saveFile->Close();

  // and in a pdf file
  cPt->Print(Form("results-pt-fit-%s.pdf[",gConfigName.c_str()));
  cPt->Print(Form("results-pt-fit-%s.pdf",gConfigName.c_str()));
  cCm->Print(Form("results-pt-fit-%s.pdf",gConfigName.c_str()));
  cCm->Print(Form("results-pt-fit-%s.pdf]",gConfigName.c_str()));
}
